...
import { Toaster, createToast } from '@/components/ui/toaster';
import {
  setAdminAuth,
  getAdminAuth,
  clearAdminAuth,
...
export default function AdminPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username === 'heroadmin' && password === 'p@s@s@w@0@r@d') {
      setAdminAuth('admin_authenticated_token_secure_12345');
      setIsLoggedIn(true);
      setUsername('');
      setPassword('');
      setProducts(getProducts());
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        createToast(`Product "${formData.name}" updated successfully!`, 'success');
      } else {
        ...
        createToast(`Product "${formData.name}" added successfully!`, 'success');
      }
      ...
      createToast(err instanceof Error ? err.message : 'Failed to save product', 'error');
    } finally {
